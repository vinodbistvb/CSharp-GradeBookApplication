    ...
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            else
            {
                ...
                if (String.Equals(parts[2],"standard"))
                {
                    StandardGradeBook gradeBook = new StandardGradeBook(parts[1], bool.Parse(parts[3]));
                    Console.WriteLine("Created gradebook {0}.", name);
                    GradeBookUserInterface.CommandLoop(gradeBook);
                }
                else if (String.Equals(parts[2],"ranked"))
                {
                    RankedGradeBook gradeBook = new RankedGradeBook(parts[1], bool.Parse(parts[3]));
                    Console.WriteLine("Created gradebook {0}.", name);
                    GradeBookUserInterface.CommandLoop(gradeBook);
                }
                else
